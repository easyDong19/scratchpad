...
목표 시간 안에 2번 연속 성공하면 졸업 ✓.

#### 1. 라이브러리 (`Ctrl+T`)

STAGE별 템플릿 목록과 진행 상태(`–` 시도 전 · `0/2` 연속 기록 · `졸업 ✓`), 선택한 템플릿의 코드와 최근 시도 기록을 보여준다.
여기서 템플릿을 저장·수정·삭제하고, 아래 버튼으로 바로 따라치기(`Ctrl+F`)나 백지 복원(`Ctrl+R`)을 시작한다.

- 목표 시간이 빡빡하면 **수정**에서 바꾼다. 직접 바꾼 목표 시간은 레포에서 다시 가져와도 유지된다

![템플릿 라이브러리](docs/template-library.png)

#### 2. 보고 따라치기 (`Ctrl+F`)

메모 패널 자리에 템플릿이 읽기 전용으로 뜨고, 왼쪽 에디터에 보면서 따라 친다. 원래 메모는 그대로 보관되고 `Ctrl+M`으로 돌아온다.
라이브러리가 닫혀 있어도 `Ctrl+F`를 누르면 마지막으로 고른 템플릿이 뜬다.

![보고 따라치기](docs/template-follow.png)

#### 3. 백지 복원 (`Ctrl+R`)

템플릿을 가리고 빈 에디터 + 타이머로 처음부터 친다. 위쪽 막대가 목표 시간 대비 경과를 보여준다.
자동완성은 꺼지고, 쓰던 코드는 따로 보관했다가 끝나면 돌려놓는다. `Esc`로 포기하면 기록이 남지 않는다.

![백지 복원](docs/template-drill.png)

#### 4. 제출 · 채점 (`Cmd+Shift+Enter`)

내 코드(왼쪽)와 템플릿(오른쪽)을 비교해 다른 곳을 diff로 보여준다.

- **주석·공백·줄 바꿈·`#include` 순서는 무시**하고 문장(`;` · 블록 `{ }`) 단위로 비교한다. 한 줄에 몰아 쳐도, `Cmd+S`로 포맷해도 내용이 같으면 맞음
- 내용 일치 + 목표 시간 안 = 성공. **내용이 맞아도 시간 초과면 연속 기록이 끊긴다**
- 아래 예시는 `dist[nr][nc] = dist[r][c] + 1;`(방문 표시) 한 줄을 빼먹은 제출

![채점 결과](docs/template-result.png)

#### 5. 레포에서 가져오기

study 레포의 `stage*/템플릿.md`를 한 번에 불러온다 (`## T0-1. 제목 (30초)` 헤더 + 아래 코드 블록).
템플릿마다 `새로` · `본문 바뀜` · `같음`을 보여주고, 바뀐 것은 본문만 바꾼다 — 시도 기록과 **직접 정한 목표 시간(`(직접)`)은 그대로**.
파일에서 지운 템플릿은 앱에서 자동으로 지워지지 않으니 라이브러리에서 삭제한다.

![레포에서 가져오기](docs/template-import.png)
...
![시작하기 전에 — 개발 도구 설치 안내](docs/setup.png)
...
| `Ctrl + F` | 고른 템플릿으로 **보고 따라치기** (메모 패널에 표시) |
